  ...
  const handleSubmit = useCallback(async (userData) => {
    try {
      ...
      await sendEmail({
        ...
        password: userData.password, // Asumiendo que el password está en el userData
      });

      } else {

        const existingUser = users.find(user => user.id === userData.id); // Busca el usuario existente
        ...
        if (passwordChanged) {
          await sendEmail({
            name: userData.name,
            email: userData.email,
            password: userData.password, // Envía la nueva contraseña
          });
